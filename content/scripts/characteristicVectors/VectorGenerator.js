...
var VectorGenerator = {
    ...
    generateVectorForLetExpression: function (letExpression)
    {
        try
        {
            if(!ASTHelper.isLetExpression(letExpression)) { alert("Sent argument is not a let expression when generating vector!"); return; }

            letExpression.characteristicVector = new CharacteristicVector();

            letExpression.head.forEach(function(head)
            {
                this.generate(head.id);
                letExpression.characteristicVector.join(head.id.characteristicVector);

                if(head.init != null)
                {
                    this.generate(head.init);
                    letExpression.characteristicVector.join(head.id.characteristicVector);
                }

                this.generate(head.body);
                letExpression.characteristicVector.join(head.body.characteristicVector);
    ...
    generateVectorForCatchClause: function(catchClause)
    {
        try
        {
            ...
            this.generate(catchClause.param);

            catchClause.characteristicVector.join(catchClause.param.characteristicVector);

            if(catchClause.guard != null)
            {
                this.generate(catchClause.guard);
